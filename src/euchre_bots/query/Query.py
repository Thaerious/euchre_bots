...
from euchre_bots.utility.Has_Hooks import HasHooks

from .compare_cards import losing_card, winning_card
...
class QueryDeck(QueryPart):
    def __init__(self, default=STATES["set"]):
        self.default = STATES["set"]
        QueryPart.__init__(self, len(INT_TO_CARD), default)
    ...
    def test_card(self, card: Card):
        ...
        if norm_card == "J♣":
            if card.trump is None:
                return self.values[norm_index] == STATES["set"]
            else:
                return self.flag_left_bower
    ...
    def _select(self, phrase):
        ...
        if len(suits) == 0:
            return self._set_cards(ranks, SUITS.keys())
        elif len(ranks) == 0:
            return self._set_cards(RANKS.keys(), suits)
        else:
            return self._set_cards(ranks, suits)
    ...
    def all(self, cards):
        selected = QueryCollection()
        for card in cards:
            if self.test_card(card):
...
class QueryDigit(QueryPart):
    def __init__(self, size):
        QueryPart.__init__(self, size)
...
class Query(QueryBase, HasHooks):
    def __init__(self, phrase="~", name=None, root=None):
        ...
        QueryBase.__init__(self, name)
        HasHooks.__init__(self)
    ...
    def action_skip(self, snap, reason, selected):
        ...
        return QueryResult("rejected", None, selected, reason)
    ...
    def _decide(self, snap: Snapshot):
        ...
        if not self._up_card.test_card(snap.up_card):
            return self.action_skip(snap, "up card", selected)

        if not self._down_card.test_card(snap.down_card):
            return self.action_skip(snap, "down card", selected)

        norm_lead = normalize_value(snap.for_index, snap._lead_index)
        if not self._lead.test_card(norm_lead):
            return self.action_skip(snap, "lead", selected)

        norm_maker = normalize_value(snap.for_index, snap.maker_index)
        if not self._maker.test_card(norm_maker):
            return self.action_skip(snap, "maker", selected)

        norm_dealer = normalize_value(snap.for_index, snap._dealer_index)
        if not self._dealer.test_card(norm_dealer):
        ...
        if self._next is not None:
            ...
            if self.data is not None:
                query_result = QueryResult(
                    self.action, self.data, selected, "data provided"
                )
            elif self._best:
                query_result = QueryResult(
                    self.action, self.do_best(selected, snap), selected, "best"
                )
            elif self._worst:
                query_result = QueryResult(
                    self.action, self.do_worst(selected, snap), selected, "worst"
                )
            else:
                query_result = QueryResult(
                    self.action, selected[0], selected, "default"
                )

            self._trigger_hook("on_match", query=self, snap=snap, result=query_result)
            return query_result

    def do_playable(self, all: QueryCollection, snap: Snapshot):
    ...
    def do_best(self, all, snap: Snapshot):
        ...
        best = all[0]
        for card in all:
            best = winning_card(lead_suit, best, card)
    ...
    def do_worst(self, all, snap: Snapshot):
        ...
        worst = all[0]
        for card in all:
            worst = losing_card(lead_suit, worst, card)
